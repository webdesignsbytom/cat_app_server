...
export const moveVideoToApprovedBucket = async (sourceBucket, destinationBucket, objectName) => {
  console.log('>>>>>>>>>>>>>>>>>>>>>>');
  ...
  try {
    ...
    const newObjectName = objectName.replace('review/', 'approved/');  // New destination path inside the bucket
    ...
    await minioClient.copyObject(bucketName, `videos/${newObjectName}`, copySource);
    console.log('<<<<<<<<<<<<<<<<<<<<');
    
    // Remove the original object from the "review" folder
    await minioClient.removeObject(bucketName, `videos/${objectName}`);
